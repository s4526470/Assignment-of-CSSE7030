...
class GridCrafterView(tk.Frame):
    ...
    def __init__(self, master, input_size):
        ...
        self._out_widgets = ItemGridView(self, (1,1))
        self._out_widgets.pack(side=tk.RIGHT)
        made_button = tk.Button(self, text="=> Craft =>")
        made_button.pack(side=tk.RIGHT)
        self._input_widgets = ItemGridView(self, input_size)
        self._input_widgets.pack(side=tk.RIGHT)
    ...
    def render(self, key_stack_pairs, selected):
        ...
        for key, stack in key_stack_pairs:
            ...
            if key == "output":
                ...
                self._out_widgets.draw_cell((0,0), stack, key == selected)
      
            else:
               
                self._input_widgets.draw_cell(key, stack, key == selected)
